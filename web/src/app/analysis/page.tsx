...
export default function AnalysisPage() {
  ...
  const statistics = {
    ...
    avgAge: filteredData.length > 0 
      ? Math.round(filteredData.reduce((sum, member) => sum + (Number(member.age) || 0), 0) / filteredData.length) 
      : 0,
    avgDuration: filteredData.length > 0 
      ? Math.round(filteredData.reduce((sum, member) => sum + (Number(member.duration_days) || 0), 0) / filteredData.length) 
      : 0,
    avgBoneLoss: filteredData.length > 0 
      ? Math.round(filteredData.reduce((sum, member) => sum + (Number(member.bone_density_change) || 0), 0) / filteredData.length * 100) / 100 
      : 0,
    avgMuscleLoss: filteredData.length > 0 
      ? Math.round(filteredData.reduce((sum, member) => sum + (Number(member.muscle_mass_change) || 0), 0) / filteredData.length * 100) / 100 
      : 0,
    countries: [...new Set(filteredData.map(member => member.country).filter(Boolean))].length,
    missions: [...new Set(filteredData.map(member => member.mission_type).filter(Boolean))].length,
    roles: [...new Set(filteredData.map(member => member.role).filter(Boolean))].length
  };
      ...
      <div className="w-full px-6 sm:px-8 lg:px-12 relative z-10">
        ...
        <div className="mb-12">
          ...
          {(activeFilters.missions.length > 0 || activeFilters.roles.length > 0 || activeFilters.countries.length > 0 || 
            ...
            <div className="bg-yellow-400/10 border border-yellow-400/30 rounded-lg p-4 mb-8">
              <h3 className="text-sm font-medium text-yellow-400 mb-2">{t('analysis.activeFilters')}</h3>
        ...
        <section className="mb-16">
          <h2 className="text-3xl font-bold text-yellow-400 mb-8 flex items-center space-x-3 font-orbitron">
            ...
            <span>{t('analysis.realtimeStatistics')}</span>
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="card-cosmic p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-3xl font-bold text-cosmic-white">{statistics.totalCrew || 0}</span>
              </div>
              <h3 className="text-lg font-semibold text-cosmic-white">{t('analysis.totalAstronauts')}</h3>
              <p className="text-cosmic-white/60 text-sm">{t('analysis.matchingCurrentFilters')}</p>
            </div>

            <div className="card-cosmic p-6">
              <div className="flex items-center justify-between mb-4">
                <Activity className="w-8 h-8 text-green-400" />
                <span className="text-3xl font-bold text-cosmic-white">{statistics.avgAge || 0}</span>
              </div>
              <h3 className="text-lg font-semibold text-cosmic-white">{t('analysis.averageAge')}</h3>
              <p className="text-cosmic-white/60 text-sm">{t('analysis.yearsAtMissionStart')}</p>
            </div>

            <div className="card-cosmic p-6">
              <div className="flex items-center justify-between mb-4">
                <BarChart3 className="w-8 h-8 text-purple-400" />
                <span className="text-3xl font-bold text-cosmic-white">{statistics.avgDuration || 0}</span>
              </div>
              <h3 className="text-lg font-semibold text-cosmic-white">{t('analysis.averageDuration')}</h3>
              <p className="text-cosmic-white/60 text-sm">{t('analysis.daysInSpace')}</p>
            </div>

            <div className="card-cosmic p-6">
              <div className="flex items-center justify-between mb-4">
                <TrendingUp className="w-8 h-8 text-yellow-400" />
                <span className="text-3xl font-bold text-cosmic-white">{statistics.countries || 0}</span>
              </div>
              <h3 className="text-lg font-semibold text-cosmic-white">{t('analysis.countries')}</h3>
              <p className="text-cosmic-white/60 text-sm">{t('analysis.representedInData')}</p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-red-400 font-semibold">{(statistics.avgBoneLoss || 0).toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-cosmic-white/10 rounded-full h-2">
                    <div 
                      className="bg-red-400 h-2 rounded-full" 
                      style={{ width: `${Math.abs(statistics.avgBoneLoss || 0) * 10}%` }}
                    ></div>
                  </div>
                </div>
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-orange-400 font-semibold">{(statistics.avgMuscleLoss || 0).toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-cosmic-white/10 rounded-full h-2">
                    <div 
                      className="bg-orange-400 h-2 rounded-full" 
                      style={{ width: `${Math.abs(statistics.avgMuscleLoss || 0) * 10}%` }}
                    ></div>
                  </div>
                </div>
              </div>
            ...
            <div className="card-cosmic p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-blue-400 font-semibold">{statistics.missions || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-cosmic-white/80">Crew Roles</span>
                  <span className="text-green-400 font-semibold">{statistics.roles || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-cosmic-white/80">Countries Represented</span>
                  <span className="text-purple-400 font-semibold">{statistics.countries || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-cosmic-white/80">Total Crew Members</span>
                  <span className="text-yellow-400 font-semibold">{statistics.totalCrew || 0}</span>
                </div>
              </div>
            </div>
          </div>
